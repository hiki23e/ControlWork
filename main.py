...
def note(*args, update_func=None):
    ...
    def save(*_):
        ...
        if not entry.get():
            showerror("Error", "Укажите название заметки")
        elif not text.get("0.0", END).strip():
            showerror("Error", "Укажите содержимое заметки")
        else:
            if args:
...
def delete(*args, update_func=None):
    ...
    if result:
        ...
        showinfo("Заметка удалена", message=f"Заметка {select_note[1]} удалена")
